...
// Relevé fournisseur imprimable (CDC digitalisation §5.3 / CDC Comptabilité
// §17), symétrique du relevé client (components/ReleveCompteClient.tsx) —
// composant partagé entre rôles (comptable, logistique) : un seul moteur.
...
interface ReleveCompteFournisseurProps {
  fournisseurId: string;
  /** Préfixe des routes API (ex. /api/comptable/fournisseurs, /api/logistique/fournisseurs). */
  apiBase: string;
}

export default function ReleveCompteFournisseur({ fournisseurId, apiBase }: ReleveCompteFournisseurProps) {
  const { data } = useApi<ReleveResponse>(`${apiBase}/${fournisseurId}/releve`);
  const r = data?.data;
