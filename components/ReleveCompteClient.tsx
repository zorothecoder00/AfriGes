...
// Relevé de compte client imprimable (CDC digitalisation §5.4 / CDC Comptabilité
// §16) — mêmes données que le grand livre auxiliaire de la fiche client
// (lib/comptabilite/clientAuxiliaire.ts), présentées comme un document à
// remettre au client (window.print(), même approche que les autres documents
// imprimables du projet — pas de génération PDF serveur pour ce document).
// Composant partagé entre les rôles (comptable, admin, RVC, agent terrain) —
// un seul moteur de relevé, pas de doublon ; seul `apiBase` change.
...
interface ReleveCompteClientProps {
  clientId: string;
  /** Préfixe des routes API (ex. /api/admin/clients, /api/rvc/clients, /api/agentTerrain/clients, /api/comptable/clients). */
  apiBase: string;
}

export default function ReleveCompteClient({ clientId, apiBase }: ReleveCompteClientProps) {
  const { data } = useApi<ReleveResponse>(`${apiBase}/${clientId}/releve`);
  const r = data?.data;
