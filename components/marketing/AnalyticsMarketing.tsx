...
interface AnalyticsData {
  ...
  recommandations: Recommandation[]; parCanal: LigneCanal[]; produits: LigneProduit[];
  complementaires: PaireComplementaire[]; saisonniers: ProduitSaisonnier[]; modele: string;
}
...
export default function AnalyticsMarketing() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-4">
        ...
        {data.produits.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            ...
            <p className="text-[10px] text-slate-400 mt-2">Rotation faible (&lt; 0,2) mise en évidence en orange.</p>
          </div>
        )}
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-4">
        <h3 className="text-sm font-semibold text-slate-700 mb-3">Produits saisonniers (12 derniers mois)</h3>
        {data.saisonniers.length === 0 ? (
          <p className="text-xs text-slate-400">Aucun produit avec un pic saisonnier net détecté</p>
        ) : (
          <div className="space-y-1.5">
            {data.saisonniers.map((s) => (
              <div key={s.produitId} className="flex items-center justify-between text-xs px-2 py-1.5 bg-slate-50 rounded-lg">
                <span className="text-slate-700">{s.nom}</span>
                <span className="text-slate-500">Pic en <span className="font-semibold text-slate-700">{s.moisPic}</span> · {s.partMoisPic}% des ventes annuelles</span>
              </div>
            ))}
          </div>
        )}
