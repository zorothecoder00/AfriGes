    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="p-6 max-w-4xl mx-auto space-y-5">
        ...
        {loading ? (
          ...
          <div id="zone-impression" className="bg-white rounded-2xl border border-slate-200 p-8 print:border-0 print:p-0">
            {/* En-tête société */}
            <div className="flex items-start justify-between border-b-2 border-slate-800 pb-3 mb-4">
              <div>
                <h1 className="text-xl font-extrabold text-slate-900">{SOCIETE.nom}</h1>
                <p className="text-[11px] text-slate-500 italic">{SOCIETE.baseline}</p>
              </div>
              <div className="text-right">
                <h2 className="text-lg font-bold text-slate-800">Catalogue produits</h2>
                <p className="text-[11px] text-slate-500">{new Date().toLocaleDateString("fr-FR")} · {rows.length} produit(s)</p>
              </div>
            </div>
              ...
              <tbody>
                {rows.map((p) => (
                  <tr key={p.id} className="border-b border-slate-100 align-top">
                    <td className="py-1.5 pr-2 font-mono text-[11px] text-slate-500 whitespace-nowrap">{p.codeProduit ?? "—"}</td>
                    <td className="py-1.5 pr-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-medium text-slate-800">{p.nomCommercial || p.nom}</span>
                      </div>
                    </td>
                    <td className="py-1.5 pr-2 text-[11px] text-slate-500">{[p.famille?.nom, p.categorieProduit?.nom, p.marque?.nom].filter(Boolean).join(" · ") || "—"}</td>
                    <td className="py-1.5 pl-2 text-right font-semibold text-slate-900 whitespace-nowrap">{formatCurrency(p.prixUnitaire)}</td>
                  </tr>
                ))}
              </tbody>
            </table>

            <p className="text-[9px] text-slate-400 text-center mt-6 pt-3 border-t border-slate-200">{SOCIETE_PIED}</p>
          </div>
        )}
      </div>
    </div>
