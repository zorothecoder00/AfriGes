        ...
        <div className="p-6 max-w-5xl mx-auto space-y-5">
          <div className="flex items-center justify-between flex-wrap gap-3 no-print">
            ...
            <div className="flex items-center gap-3">
              <span className="text-sm text-slate-400">{etiquettes.length} étiquette(s)</span>
              <button onClick={() => window.print()} className="inline-flex items-center gap-2 px-4 py-2.5 bg-primary-600 hover:bg-primary-700 text-white rounded-xl text-sm font-semibold shadow-sm">
          ...
          <div id="zone-impression">
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              {etiquettes.map((e) => (
                <div key={e.id} className="border border-slate-300 rounded-lg p-3 flex flex-col items-center text-center bg-white" style={{ breakInside: "avoid" }}>
                  <p className="text-sm font-bold text-slate-900 leading-tight line-clamp-2">{e.nom}</p>
                  {e.marque && <p className="text-[10px] text-slate-400 uppercase">{e.marque}</p>}
                  <p className="text-lg font-extrabold text-slate-900 my-1">{formatCurrency(e.prixUnitaire)}</p>
      ...
      <div className="p-6 max-w-4xl mx-auto space-y-5">
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          ...
          <button onClick={generer} disabled={selected.size === 0 || generating}
            className="inline-flex items-center gap-2 px-4 py-2.5 bg-primary-600 hover:bg-primary-700 disabled:opacity-50 text-white rounded-xl text-sm font-semibold shadow-sm">
        ...
        <div className="relative">
          ...
          <input value={searchInput} onChange={(e) => setSearchInput(e.target.value)}
            ...
            className="w-full pl-9 pr-3 py-2.5 border border-slate-200 rounded-xl text-sm bg-white focus:outline-none focus:ring-2 focus:ring-blue-500" />
        </div>

        <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
          <div className="flex items-center justify-between px-4 py-2.5 border-b border-slate-100">
            <button onClick={toggleAllVisible} className="inline-flex items-center gap-2 text-sm text-slate-600 hover:text-slate-800">
          ...
          {loading ? (
            ...
            <div className="py-16 text-center text-slate-400">Aucun produit.</div>
          ) : (
              ...
              {rows.map((p) => {
                ...
                return (
                  <button key={p.id} onClick={() => toggle(p.id)}
                    className={`w-full flex items-center gap-3 px-4 py-2.5 text-left hover:bg-slate-50 ${sel ? "bg-blue-50/50" : ""}`}>
                    {sel ? <CheckSquare className="w-4 h-4 text-blue-600 shrink-0" /> : <Square className="w-4 h-4 text-slate-300 shrink-0" />}
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="text-sm font-medium text-slate-800 truncate">{p.nomCommercial || p.nom}</p>
                      <p className="text-[11px] text-slate-400 font-mono">{p.codeProduit ?? "—"}{p.marque ? ` · ${p.marque.nom}` : ""}</p>
                    </div>
                    <span className="text-sm font-semibold text-slate-700">{formatCurrency(p.prixUnitaire)}</span>
                  </button>
                );
              })}
            </div>
