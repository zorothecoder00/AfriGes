...
function Kpi({ icon, label, value, tone }: { icon: React.ReactNode; label: string; value: string; tone: string }) {
  ...
  return (
    <div className={`group relative overflow-hidden ${(STAT_CARD_HUES[hue] ?? STAT_CARD_HUES.slate).solid} rounded-2xl border p-4 transition-all duration-300 ease-out hover:-translate-y-1 hover:shadow-xl`}>
      <div className={`inline-flex items-center justify-center w-9 h-9 rounded-lg mb-2 transition-transform duration-300 ease-out group-hover:scale-110 bg-white/20 text-white [&_svg]:text-white`}>{icon}</div>
