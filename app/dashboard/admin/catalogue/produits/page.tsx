      ...
      <div className="px-3 py-4 sm:p-6 max-w-7xl mx-auto space-y-5">
          ...
          <div className="flex flex-wrap gap-2 w-full lg:w-auto">
            <div className="relative">
              <button onClick={() => setDocsOpen((o) => !o)} onBlur={() => setTimeout(() => setDocsOpen(false), 150)}
                className="flex-1 sm:flex-none inline-flex justify-center items-center gap-2 px-4 py-2.5 bg-white border border-slate-200 text-slate-700 rounded-xl text-sm font-medium hover:bg-slate-50">
            ...
            <button onClick={() => { setEditId(null); setModalOpen(true); }}
              className="flex-1 sm:flex-none inline-flex justify-center items-center gap-2 px-4 py-2.5 bg-primary-600 hover:bg-primary-700 text-white rounded-xl text-sm font-semibold shadow-sm">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
          ...
          {(statut || familleId || categorieId || marqueId || search) && (
            <button onClick={() => { setStatut(""); setFamilleId(""); setCategorieId(""); setMarqueId(""); setSearchInput(""); setPage(1); }}
              className="px-3 py-2.5 text-sm text-slate-500 hover:text-slate-700">Réinitialiser</button>
          )}
        </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
          {loading ? (
            ...
            <div className="py-20 text-center text-slate-400"><Filter className="w-8 h-8 mx-auto mb-2 opacity-40" /> Aucun produit trouvé.</div>
          ) : (
                ...
                <tbody className="divide-y divide-slate-50">
                  {rows.map((p) => (
                    <tr key={p.id} className="hover:bg-slate-50/60">
                      <td className="px-4 py-3">
                        <div className="flex items-start gap-3">
                          ...
                          <div className="min-w-0 flex-1">
                            <Link href={`/dashboard/admin/catalogue/produits/${p.id}`} className="font-medium text-slate-800 truncate hover:text-blue-600 hover:underline block">{p.nom}</Link>
                            <p className="text-[11px] text-slate-400 font-mono">{p.codeProduit ?? "—"}{p.reference ? ` · ${p.reference}` : ""}</p>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="px-4 py-3 text-right font-semibold text-slate-900">{formatCurrency(p.prixUnitaire)}</td>
                      <td className="px-4 py-3 text-right text-slate-500">{p.prixAchat != null ? formatCurrency(p.prixAchat) : "—"}</td>
        ...
        {totalPages > 1 && (
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
            <button disabled={page <= 1} onClick={() => setPage((p) => p - 1)} className="w-full sm:w-auto px-4 py-2 border border-slate-200 rounded-lg text-sm disabled:opacity-40 bg-white">Précédent</button>
            <span className="text-sm text-slate-500">Page {page} / {totalPages}</span>
            <button disabled={page >= totalPages} onClick={() => setPage((p) => p + 1)} className="w-full sm:w-auto px-4 py-2 border border-slate-200 rounded-lg text-sm disabled:opacity-40 bg-white">Suivant</button>
          </div>
        )}
      </div>
