    ...
    <div className="min-h-screen bg-slate-50">
      <div className="p-6 max-w-5xl mx-auto space-y-5">
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          ...
          <button onClick={telechargerModele} className="inline-flex items-center gap-2 px-4 py-2.5 bg-white border border-slate-200 text-slate-700 rounded-xl text-sm font-medium hover:bg-slate-50">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-5 space-y-4">
          <label className="flex flex-col items-center justify-center gap-2 border-2 border-dashed border-slate-200 rounded-xl py-8 cursor-pointer hover:border-blue-300 hover:bg-blue-50/30">
            <FileSpreadsheet className="w-8 h-8 text-slate-300" />
            <span className="text-sm text-slate-600">{fileName || "Cliquez pour choisir un fichier .xlsx ou .csv"}</span>
          ...
          <div className="flex items-center justify-between flex-wrap gap-3">
            <label className="flex items-center gap-2 text-sm text-slate-600">
              Rapprochement par
              <select value={cle} onChange={(e) => setCle(e.target.value as "codeProduit" | "reference")} className="px-2 py-1.5 border border-slate-200 rounded-lg bg-white">
            ...
            <div className="flex gap-2">
              <button onClick={() => lancer("dry-run")} disabled={lignes.length === 0 || busy}
                className="inline-flex items-center gap-2 px-4 py-2.5 bg-white border border-slate-200 text-slate-700 rounded-xl text-sm font-medium hover:bg-slate-50 disabled:opacity-50">
              ...
              <button onClick={() => lancer("apply")} disabled={lignes.length === 0 || busy}
                className="inline-flex items-center gap-2 px-4 py-2.5 bg-primary-600 hover:bg-primary-700 text-white rounded-xl text-sm font-semibold disabled:opacity-50">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
            <div className="px-5 py-3 border-b border-slate-100 flex items-center justify-between flex-wrap gap-2">
              <h3 className="font-semibold text-slate-800 flex items-center gap-2">
                {applied ? <CheckCircle2 className="w-4 h-4 text-emerald-600" /> : <Play className="w-4 h-4 text-slate-500" />}
              ...
              <div className="flex items-center gap-3 text-sm">
                ...
                <span className={resultat.resume.erreurs ? "text-rose-600" : "text-slate-400"}>{resultat.resume.erreurs} erreur(s)</span>
              </div>
            </div>
            <div className="overflow-x-auto max-h-[55vh]">
              <table className="w-full text-sm">
                <thead className="bg-slate-50 text-slate-400 text-xs uppercase sticky top-0">
                  <tr>
                ...
                <tbody className="divide-y divide-slate-50">
                  {resultat.rapports.map((r) => (
                    <tr key={r.ligne} className={r.action === "error" ? "bg-rose-50/40" : ""}>
                      <td className="px-4 py-2 text-slate-400">{r.ligne}</td>
                      ...
                      <td className="px-4 py-2 text-slate-800">{r.nom || "—"}</td>
                      <td className="px-4 py-2 font-mono text-[11px] text-slate-500">{r.codeProduit ?? "—"}</td>
                      <td className="px-4 py-2 text-slate-500 text-xs">{r.message}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
