...
const PRIORITE_COLOR: Record<string, string> = {
  BASSE: "text-slate-400", NORMALE: "text-blue-600", HAUTE: "text-amber-600", URGENTE: "text-rose-600",
};
...
export default function ResolutionDetailPage() {
  ...
  const [planForm, setPlanForm] = useState({ titre: "", description: "", priorite: "NORMALE", dateEcheance: "" });
  ...
  async function soumettrePlan(e: React.FormEvent) {
    ...
    if (r?.id) {
      ...
      setPlanForm({ titre: "", description: "", priorite: "NORMALE", dateEcheance: "" });
      setRefresh(x => x + 1);
    } else toast.error(r?.error || "Erreur");
  }
          ...
          {res.dateEcheance && (
            <div className="flex items-center gap-2 text-sm text-slate-600">
              ...
              <div>
                ...
                <p className={`font-medium ${new Date(res.dateEcheance) < new Date() && res.statut !== "IMPLEMENTEE" ? "text-rose-600" : ""}`}>
