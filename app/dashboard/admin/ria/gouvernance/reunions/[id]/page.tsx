...
const STATUT_RES: Record<string, { label: string; color: string }> = {
  EN_ATTENTE:     { label: "En préparation", color: "bg-slate-100 text-slate-600" },
  EN_COURS:       { label: "Soumise",        color: "bg-blue-100 text-blue-700" },
  MISE_EN_OEUVRE: { label: "Adoptée",        color: "bg-emerald-100 text-emerald-700" },
  ABANDONNEE:     { label: "Rejetée",        color: "bg-rose-100 text-rose-700" },
  IMPLEMENTEE:    { label: "Exécutée",       color: "bg-violet-100 text-violet-700" },
};
...
function OngletPlansAction({ r }: { r: Reunion }) {
  ...
  const [form, setForm] = useState({ titre: "", description: "", priorite: "NORMALE", dateEcheance: "", progression: "0" });
  ...
  async function soumettre(e: React.FormEvent) {
    ...
    if (res?.id) { toast.success("Plan d\'action créé"); setShowForm(false); setRefresh(x => x + 1); setForm({ titre: "", description: "", priorite: "NORMALE", dateEcheance: "", progression: "0" }); }
    else toast.error(res?.error || "Erreur");
  }
