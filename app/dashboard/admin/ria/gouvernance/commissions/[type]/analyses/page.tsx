...
function CreateModal({ defaultRapportId, onClose, onDone }: { defaultRapportId?: string; onClose: () => void; onDone: () => void }) {
  ...
  const { mutate, loading } = useMutation("/api/admin/ria/commissions/gouvernance/analyses-optimisation", "POST");
...
function AnalyseRow({ item, expanded, onToggle, onSaved }: { item: Analyse; expanded: boolean; onToggle: () => void; onSaved: () => void }) {
  const { mutate, loading } = useMutation(`/api/admin/ria/commissions/gouvernance/analyses-optimisation/${item.id}`, "PATCH");
  const { mutate: transformer, loading: transforming } = useMutation(`/api/admin/ria/commissions/gouvernance/analyses-optimisation/${item.id}/transformer-suggestion`, "POST");
...
export default function AnalysesOptimisationPage() {
  ...
  const { data, loading, refetch } = useApi<Data>(`/api/admin/ria/commissions/gouvernance/analyses-optimisation?${params.toString()}`);
