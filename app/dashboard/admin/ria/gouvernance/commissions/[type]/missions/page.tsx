...
function CreateModal({ defaultFinancementId, defaultDossierICId, onClose, onDone }: {
  ...
  const { mutate, loading } = useMutation("/api/admin/ria/commissions/gouvernance/missions-audit", "POST");
...
function MissionRow({ mission, expanded, onToggle, onSaved }: {
  ...
  const { mutate, loading } = useMutation(`/api/admin/ria/commissions/gouvernance/missions-audit/${mission.id}`, "PATCH");
...
export default function MissionsAuditPage() {
  ...
  const { data, loading, refetch } = useApi<Data>(`/api/admin/ria/commissions/gouvernance/missions-audit?${params.toString()}`);
