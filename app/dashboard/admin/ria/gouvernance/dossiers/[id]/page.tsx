          ...
          {def.isApprouver && (
            <>
              ...
              <div>
                ...
                <PortefeuilleSelect apiBase="/api/admin/ria/portefeuilles" value={portefeuilleExecutionId}
...
function FormulaireFinancement({ dossier, editable, onSaved }: { dossier: DossierDetail; editable: boolean; onSaved: () => void }) {
  ...
  const { mutate, loading } = useMutation(`/api/admin/ria/commissions/gouvernance/dossiers/${dossier.id}`, "PATCH");
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 p-5 space-y-4">
      ...
      <DemandeFinancementEditor value={contenu} onChange={handleChange} disabled={!editable} />
    </div>
  );
}
...
export default function DossierDetailPage() {
  ...
  const { data: dossier, loading, refetch } = useApi<DossierDetail>(`/api/admin/ria/commissions/gouvernance/dossiers/${id}`);
  const { mutate: mutatePatch, loading: actionEnCours } = useMutation(`/api/admin/ria/commissions/gouvernance/dossiers/${id}`, "PATCH");
  const { mutate: mutateEchange, loading: envoiEnCours } = useMutation(`/api/admin/ria/commissions/gouvernance/dossiers/${id}/echanges`, "POST");
    ...
    <div className="md:p-6 space-y-6 max-w-5xl mx-auto">
      ...
      {isFinancement && dossier.statut === "APPROUVE" && (
        <div className="rounded-xl border border-emerald-300 bg-emerald-50 p-4 flex items-start gap-3">
          ...
          {peutDecaisser ? (
            <button onClick={() => runAction({ action: "EXECUTER" })} disabled={actionEnCours}
              ...
              <Wallet className="w-4 h-4" /> {actionEnCours ? "Décaissement..." : "Décaisser maintenant"}
            </button>
          ) : (
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-5">
        ...
        <div className="flex gap-2">
          <input value={nouvelEchange} onChange={e => setNouvelEchange(e.target.value)}
            ...
            className="flex-1 border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-violet-300 min-w-0" />
          <button onClick={envoyerEchange} disabled={envoiEnCours || !nouvelEchange.trim()}
