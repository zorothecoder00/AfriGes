...
interface RapportDetaille {
  ...
  donnees: {
    ...
    financementsActifs: number;
    financementsEnRetard: number;
    financementsRembourses: number;
...
export default function BulletinRIAPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (error || !data?.rapport) return (
    <div className="p-8 text-red-600">Rapport introuvable.</div>
  );
            ...
            <table className="w-full text-sm">
              <tbody>
                {[
                  ...
                  ["Taux généré (" + d.distribution.capitalBase + ")",              "", ""],
                  ["Montant généré (10%)",   fmt(d.distribution.montantGenere),    "font-medium text-slate-800"],
                  ["→ Part investisseur (4%)", fmt(d.distribution.montantDistribue), "text-emerald-700 pl-4"],
                  ["→ Réinvesti (4%)",        fmt(d.distribution.montantReinvesti), "text-blue-600 pl-4"],
                  ["→ Fonds sécurité (2%)",   fmt(d.distribution.montantSecurite),  "text-amber-600 pl-4"],
                ].filter(([, v]) => v !== "").map(([label, valeur, cls]) => (
                  <tr key={label} className="border-b border-slate-50">
                    <td className="py-1.5 text-slate-500">{label}</td>
                    <td className={`py-1.5 text-right ${cls}`}>{valeur}</td>
                  </tr>
