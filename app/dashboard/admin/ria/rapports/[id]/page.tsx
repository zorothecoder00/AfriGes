...
import { RefreshCw, Printer, FileSpreadsheet, ArrowLeft } from "lucide-react";
...
interface RapportData {
  data: {
    ...
    portefeuille: {
      ...
      profilRIA: { numero: string | null; gestionnaire: { member: { nom: string; prenom: string; email: string; telephone: string | null } } | null } | null;
    };
  };
}
...
export default function RapportDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  const { data, loading, error } = useApi<RapportData>(`/api/admin/ria/rapports/${id}`);
  ...
  if (error || !data?.data) {
    return <div className="p-8 text-red-600">Rapport introuvable.</div>;
  }
  ...
  return (
    <>
      ...
      <div className="print:hidden sticky top-0 z-10 bg-white border-b border-slate-200 px-6 py-3 flex items-center gap-3">
        <Link href="/dashboard/admin/ria/rapports"
        ...
        <div className="flex-1">
          ...
          <p className="text-xs text-slate-500">{d.portefeuille?.investisseur}</p>
        </div>
