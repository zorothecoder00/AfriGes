...
function PlanifierModal({ onClose, onSuccess }: { onClose: () => void; onSuccess: () => void }) {
  ...
  const submit = async () => {
    if (!form.portefeuilleId || !form.mois || !form.annee) {
            ...
            <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4 space-y-1.5">
              ...
              <div className="border-t border-emerald-200 pt-1.5 space-y-1">
                <div className="flex justify-between text-xs text-slate-500">
                  <span>→ Distribué ({toNum(cfg?.tauxDistribue)}%)</span>
                  <span>{fmt(preview.distribue)} F</span>
                </div>
                <div className="flex justify-between text-xs text-slate-500">
                  <span>→ Réinvesti ({toNum(cfg?.tauxReinvesti)}%)</span>
                  <span>{fmt(preview.reinvesti)} F</span>
                </div>
                <div className="flex justify-between text-xs text-slate-500">
                  <span>→ Fonds sécurité ({toNum(cfg?.tauxFondSecurite)}%)</span>
                  <span>{fmt(preview.fondSec)} F</span>
                </div>
              </div>
            </div>
          )}
