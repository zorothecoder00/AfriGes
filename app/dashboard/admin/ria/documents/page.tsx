...
const TYPE_META: Record<TypeDoc, { label: string; icon: React.ReactNode; color: string; desc: string }> = {
  ...
  RELEVE_PORTEFEUILLE:     { label: "Relevé de Portefeuille",  icon: <BookOpen size={14} />,     color: "bg-teal-50 text-teal-700 border-teal-200",    desc: "Situation complète d'un portefeuille" },
  ...
  RAPPORT_RENTABILITE:     { label: "Rapport Rentabilité",     icon: <TrendingUp size={14} />,   color: "bg-indigo-50 text-indigo-700 border-indigo-200", desc: "Analyse ROI & rendement" },
  RAPPORT_RISQUE:          { label: "Rapport de Risque",       icon: <Shield size={14} />,       color: "bg-red-50 text-red-700 border-red-200",       desc: "Évaluation des risques & créances" },
    ...
    <div className="p-6 max-w-6xl mx-auto space-y-6">
      ...
      {liste.length === 0 ? (
        <div className="bg-white rounded-xl border border-slate-200 p-12 text-center">
          ...
          <p className="text-sm text-slate-400 mt-1">Cliquez sur &quot;Générer un document&quot; pour créer le premier document.</p>
        </div>
      ) : (
        <div className="bg-white rounded-xl border border-slate-200 divide-y divide-slate-100">
          {liste.map((doc) => (
            <Link key={doc.id} href={`/dashboard/admin/ria/documents/${doc.id}`}
...
function GenererModal({ onClose, onSuccess }: { onClose: () => void; onSuccess: () => void }) {
  ...
  async function handleGenerer() {
    if (!type) { toast.error("Sélectionnez un type de document"); return; }
