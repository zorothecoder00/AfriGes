...
function BarChart({ items, max, color = "#10b981" }: {
  ...
  const W = 480; const H = 160; const PAD = 40; const BAR_W = 28; const GAP = 8;
      ...
      {items.map((item, i) => {
        ...
        return (
          <g key={i}>
            ...
            <text x={x + BAR_W / 2} y={H - PAD + 14} textAnchor="middle"
              fontSize={9} fill="#64748b">{item.label}</text>
          </g>
        );
      })}
...
function KCard({ label, value, sub, accent = false }: {
...
export default function BIPage() {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      {/* Alerte encours échus */}
      {encoursEchus > 0 && (
        <div className="flex items-start gap-3 p-4 bg-red-50 border border-red-200 rounded-xl text-sm text-red-700">
          <AlertTriangle className="w-5 h-5 shrink-0 mt-0.5" />
          <span>
            <strong>{fmt(encoursEchus)}</strong> d&apos;encours sont en retard de paiement. Consultez la page
            Recouvrement pour les détails et actions.
          </span>
        </div>
      )}

      {/* Top 10 portefeuilles + Répartition risque */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white rounded-xl border border-slate-200">
          ...
          <div className="p-4">
            <BarChart
              items={top10Portefeuilles.map((p, i) => ({ label: `PF${i + 1}`, value: p.capitalInvesti }))}
              max={maxCapital}
            />
          </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200">
          ...
          <div className="p-5 space-y-3">
            ...
            {totalRisque === 0 && (
              <p className="text-sm text-slate-400 text-center py-4">Aucune affectation active</p>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200">
        ...
        <div className="p-5">
          ...
          <p className="text-xs text-slate-400 mt-4">
            Montants cumulatifs des encours sur financements actifs dont l&apos;échéance intervient avant chaque horizon.
            Les remboursements partiels déjà effectués sont déduits.
          </p>
        </div>
      </div>
      ...
      {evolutionMensuelle.length > 0 && (
        <div className="bg-white rounded-xl border border-slate-200">
          ...
          <div className="p-5">
            <BarChart
              items={evolutionMensuelle.map((e) => ({
                label: `${MOIS[e.mois]} ${String(e.annee).slice(2)}`,
                value: e.genere,
              }))}
              max={maxEvolution}
              color="#10b981"
