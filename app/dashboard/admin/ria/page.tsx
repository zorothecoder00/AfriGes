...
export default function RIADashboardPage() {
  ...
  const totalRisque = d
    ? Object.values(d.repartitionRisque).reduce((s, v) => s + v, 0)
    : 0;
    ...
    <div className="p-6 space-y-8 max-w-screen-2xl">
      ...
      {d && (
        <>
          ...
          <section>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              <KpiCard
                ...
                label="Investisseurs"
                value={String(d.nbInvestisseurs)}
                sub={`${d.nbPortefeuilles} portefeuille(s)`}
                color="violet"
              />
              <KpiCard
                icon={<UserCheck className="w-5 h-5" />}
                label="Clients financés"
                value={String(d.nbClientsFinances)}
              ...
              <KpiCard
                ...
                value={String(d.nbDossiersActifs)}
                sub="financements en cours"
                color="teal"
              />
              <KpiCard
                icon={<Wallet className="w-5 h-5" />}
                label="Capital total investi"
                value={`${fmt(d.capitalInvesti)} F`}
                color="emerald"
              />
            </div>
          </section>
          ...
          <section>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              <KpiCard
                ...
                value={`${fmt(d.capitalDisponible)} F`}
                color="emerald"
              />
              <KpiCard
                icon={<BarChart2 className="w-5 h-5" />}
                label="Encours global"
                value={`${fmt(d.encoursGlobal)} F`}
                sub="capital engagé actif"
                color="blue"
              />
              <KpiCard
                icon={<CheckCircle2 className="w-5 h-5" />}
                label="Capital recouvré"
                value={`${fmt(d.capitalRecouvre)} F`}
                color="teal"
              />
              <KpiCard
                icon={<Shield className="w-5 h-5" />}
                label="Capital immobilisé"
                value={`${fmt(d.capitalBloque)} F`}
                sub="bloqué / litige"
                color={d.capitalBloque > 0 ? "rose" : "slate"}
                alert={d.capitalBloque > 0}
              />
            </div>
          </section>
          ...
          <section>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              <KpiCard
                ...
                label="Recouvré aujourd'hui"
                value={`${fmt(d.montantRecouvreDuJour)} F`}
                color="emerald"
              />
              <KpiCard
                icon={<TrendingUp className="w-5 h-5" />}
                label="Recouvré ce mois"
                value={`${fmt(d.montantRecouvreDuMois)} F`}
                color="blue"
              />
              <KpiCard
                icon={<AlertTriangle className="w-5 h-5" />}
                label="Créances échues"
                value={String(d.nbCreancesEchues)}
                sub={`${fmt(d.montantCreancesEchues)} F en retard`}
                color={d.nbCreancesEchues > 0 ? "rose" : "slate"}
                alert={d.nbCreancesEchues > 0}
              />
              <KpiCard
                icon={<TrendingDown className="w-5 h-5" />}
                label="Taux de rotation"
                value={pct(d.tauxRotation)}
                sub="financé / capital investi"
                color="amber"
              />
            </div>
          </section>
          ...
          {/* ── Bloc 5 — Bénéfices & fond sécurité ── */}
          <section>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="bg-white rounded-2xl border border-slate-200 p-5">
              ...
              <div className="space-y-3">
                {[
                  ...
                  <RisqueBar
                    key={label}
                    label={label}
                    count={d.repartitionRisque[label] ?? 0}
                    total={totalRisque}
                    color={color}
                  />
                ))}
              ...
              {totalRisque === 0 && (
                <p className="text-sm text-slate-400 text-center mt-3">Aucune affectation active</p>
              )}
            </div>
