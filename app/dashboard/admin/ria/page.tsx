...
function KpiCard({ icon, label, value, sub, color = "slate", alert = false }: {
  ...
  return (
    <div className={`group relative overflow-hidden ${solid} rounded-2xl border p-4 sm:p-5 flex flex-col sm:flex-row items-start gap-2 sm:gap-4 min-w-0 transition-all duration-300 ease-out hover:-translate-y-1 hover:shadow-xl`}>
      ...
      <div className="min-w-0">
        <p className="text-xs text-white/80 font-medium uppercase tracking-wider [overflow-wrap:anywhere]">{label}</p>
        <p className={`text-lg sm:text-xl leading-tight [overflow-wrap:anywhere] font-bold mt-0.5 transition-transform duration-300 group-hover:scale-105 origin-left text-white`}>{value}</p>
