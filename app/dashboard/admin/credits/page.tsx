...
interface CreditDetail extends CreditClient {
  lignes: { id: number; produitNom: string; quantite: number; prixUnitaire: string | number; remise: string | number; montantLigne: string | number; produit: { id: number; nom: string } | null }[];
              ...
              <>
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-5">
                  ...
                  {detailCredit.lignes.length > 0 && (
                    <div>
                      <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Produits ({detailCredit.lignes.length})</p>
                      <div className="space-y-1">
                        {detailCredit.lignes.map((l) => (
                          <div key={l.id} className="flex items-center justify-between bg-gray-50 rounded-lg px-3 py-2 text-sm">
                            <span className="text-gray-700">{l.produitNom} <span className="text-gray-400">× {l.quantite}</span></span>
                            <span className="font-medium text-gray-800">{formatCurrency(Number(l.montantLigne))}</span>
                          </div>
                        ))}
                      </div>
                    </div>
