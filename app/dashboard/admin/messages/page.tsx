...
import { ArrowLeft, Send, MessageSquare, ChevronDown, ChevronUp, Clock } from "lucide-react";
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-start gap-3 min-w-0">
            <div className="w-10 h-10 bg-gradient-to-br from-emerald-500 to-emerald-600 rounded-full flex items-center justify-center text-white font-semibold text-sm flex-shrink-0">
              {message.destinataire.prenom[0]}{message.destinataire.nom[0]}
            </div>
          ...
          <div className="flex items-center gap-3 flex-shrink-0">
            {message.reponses.length > 0 && (
              <span className="text-xs font-medium px-2.5 py-1 bg-emerald-50 text-emerald-600 rounded-full">
        ...
        <div className="border-t border-slate-100 px-6 py-5 space-y-5">
          ...
          {message.reponses.length > 0 && (
            <div className="space-y-3">
              <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Réponses</p>
          ...
          <form onSubmit={handleReply} className="flex gap-3 items-end">
            <textarea
              ...
              className="flex-1 px-4 py-2.5 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 bg-slate-50 resize-none"
            />
            <button
              type="submit"
              disabled={loading || !replyText.trim()}
              className="px-4 py-2.5 bg-emerald-600 text-white rounded-xl hover:bg-emerald-700 transition-all font-medium text-sm flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed flex-shrink-0"
            >
              <Send size={15} />
              {loading ? "…" : "Répondre"}
            </button>
...
export default function AdminMessagesPage() {
  ...
  const { data: response, loading, error, refetch } = useApi<MessagesResponse>(
    `/api/admin/messages?page=${page}&limit=20`
  );
      ...
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1">
            <h1 className="text-3xl font-bold text-slate-800">Messages envoyés</h1>
            <p className="text-slate-500 text-sm mt-0.5">
              ...
              {(meta?.nonLus ?? 0) > 0 && (
                <span className="ml-2 px-2 py-0.5 bg-emerald-100 text-emerald-700 rounded-full text-xs font-semibold">
                  {meta!.nonLus} réponse{meta!.nonLus > 1 ? "s" : ""} non lue{meta!.nonLus > 1 ? "s" : ""}
                </span>
              )}
            </p>
          </div>
        ...
        {messages.length === 0 ? (
          <div className="bg-white rounded-2xl p-12 shadow-sm border border-slate-200/60 text-center">
            ...
            <p className="text-slate-500 font-medium">Aucun message envoyé</p>
            <p className="text-slate-400 text-sm mt-1">
              Utilisez &quot;+ Nouvelle opération&quot; pour envoyer un message à un gestionnaire.
            </p>
          </div>
        ) : (
