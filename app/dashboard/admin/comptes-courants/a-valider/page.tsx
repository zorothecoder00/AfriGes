...
export default function RetraitsAValiderPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <div className="p-6 max-w-4xl mx-auto space-y-6">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            ...
            <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2 mt-2">
              ...
              {retraits.length > 0 && (
                <Badge variant="warning">{retraits.length}</Badge>
              )}
            </h1>
            <p className="text-sm text-slate-500 mt-0.5">Demandes de retrait initiées par les caissiers, en attente de votre validation.</p>
          </div>
        ...
        {loading && !res ? (
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm text-center py-20">
            ...
            <p className="text-slate-500 font-medium">Aucun retrait en attente</p>
            <p className="text-slate-400 text-sm mt-1">Toutes les demandes ont été traitées.</p>
          </div>
        ) : (
          <div className="space-y-3">
            {retraits.map((rt) => (
              <div key={rt.id} className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 flex items-center justify-between gap-4 flex-wrap">
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="text-lg font-extrabold text-orange-700">− {formatCurrency(Math.abs(N(rt.montant)))}</span>
                    <span className="text-xs text-slate-400">→ solde après {formatCurrency(N(rt.soldeApres))}</span>
                  </div>
                  <p className="font-medium text-slate-800 mt-1">{rt.compte.client.prenom} {rt.compte.client.nom}</p>
                  ...
                  {rt.observation && <p className="text-[11px] text-slate-400 mt-0.5 italic">{rt.observation}</p>}
                </div>
                <div className="flex items-center gap-2">
                  <Button variant="success" icon={<ShieldAlert className="w-4 h-4" />} onClick={() => { setPassword(""); setVal(rt); }}>
                    Valider
                  </Button>
                  <Button variant="danger" icon={<X className="w-4 h-4" />} onClick={() => { setMotif(""); setRej(rt); }}>
                    Rejeter
                  </Button>
                </div>
              </div>
      ...
      {/* Modal validation */}
      <Modal open={!!val} onClose={() => setVal(null)} title="Valider le retrait" size="sm">
        {val && (
          <div className="space-y-4">
            <div className="bg-slate-50 rounded-xl border border-slate-200 p-4 text-sm space-y-1">
              <div className="flex justify-between"><span className="text-slate-500">Client</span><span className="text-slate-800">{val.compte.client.prenom} {val.compte.client.nom}</span></div>
              <div className="flex justify-between"><span className="text-slate-500">Montant</span><span className="font-semibold text-orange-700">− {formatCurrency(Math.abs(N(val.montant)))}</span></div>
              <div className="flex justify-between"><span className="text-slate-500">Solde après</span><span className="font-medium text-slate-800">{formatCurrency(N(val.soldeApres))}</span></div>
              <div className="flex justify-between"><span className="text-slate-500">Initié par</span><span className="text-slate-700">{val.user ? `${val.user.prenom} ${val.user.nom}` : "—"}</span></div>
            </div>
            ...
            <div className="flex gap-3 pt-2">
              <Button variant="secondary" className="flex-1" onClick={() => setVal(null)}>
                Annuler
              </Button>
              <Button variant="success" className="flex-1" loading={valSaving} icon={<ShieldAlert className="w-4 h-4" />} onClick={submitValider}>
                Valider et décaisser
              </Button>
            </div>
          </div>
        )}
      ...
      {/* Modal rejet */}
      <Modal open={!!rej} onClose={() => setRej(null)} title="Rejeter le retrait" size="sm">
        {rej && (
          <div className="space-y-4">
            <p className="text-xs text-slate-500">Retrait de <span className="font-semibold text-orange-700">{formatCurrency(Math.abs(N(rej.montant)))}</span> — {rej.reference}</p>
            <label className="block">
            ...
            <div className="flex gap-3 pt-2">
              <Button variant="secondary" className="flex-1" onClick={() => setRej(null)}>
                Annuler
              </Button>
              <Button variant="danger" className="flex-1" loading={rejSaving} icon={<X className="w-4 h-4" />} onClick={submitRejeter}>
                Confirmer le rejet
              </Button>
            </div>
          </div>
