...
interface StockItem {
  ...
  quantiteBloquee: number; quantiteConsignee: number;
...
export default function GestionStockPage() {
  ...
  const [blocageStock, setBlocageStock] = useState<{
    produitId: number; produitNom: string; pointDeVenteId: number; pointDeVenteNom: string;
    quantite: number; quantiteBloquee: number; quantiteConsignee: number;
  } | null>(null);
      ...
      <div className="max-w-[1600px] mx-auto space-y-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
          {[
            ...
            return (
              <div key={i} className={`group relative overflow-hidden ${getStatCardHue(stat.color).solid} rounded-2xl p-5 shadow-sm border transition-all duration-300 ease-out hover:-translate-y-1 hover:shadow-xl`}>
                <div className={`bg-white/20 text-white [&_svg]:text-white p-3 rounded-xl inline-block mb-3 transition-transform duration-300 ease-out group-hover:scale-110`}>
                  <Icon className="text-white w-5 h-5" />
                </div>
                <h3 className={`text-white/85 text-xs font-semibold mb-1`}>{stat.label}</h3>
                <p className={`text-xl sm:text-2xl leading-tight [overflow-wrap:anywhere] font-bold text-white transition-transform duration-300 group-hover:scale-105 origin-left`}>{stat.value}</p>
                {stat.sub && <p className="text-xs text-white/70 mt-1">{stat.sub}</p>}
              </div>
            );
          })}
        </div>
        ...
        {blocageStock && (
          <BlocageStockModal
            stock={blocageStock}
            onClose={() => setBlocageStock(null)}
            onDone={() => { setBlocageStock(null); refetch(); }}
          />
        )}
                          ...
                          <tr key={i} className="bg-blue-50/50 border-l-4 border-l-blue-200">
                            ...
                            <td className="px-5 py-2">
                              <div className="space-y-0.5">
                                ...
                                {s2.quantiteBloquee > 0 && <p className="text-xs text-orange-600 font-medium">{s2.quantiteBloquee} bloqué</p>}
                                {s2.quantiteConsignee > 0 && <p className="text-xs text-purple-600 font-medium">{s2.quantiteConsignee} consigné</p>}
                                <p className="text-xs text-slate-500">Théorique : {s2.stockTheorique}</p>
                              </div>
                            </td>
                            <td colSpan={3} />
                            <td className="px-5 py-2 text-right">
                              <button onClick={() => setBlocageStock({ produitId: p.id, produitNom: p.nom, pointDeVenteId: s2.pointDeVenteId, pointDeVenteNom: s2.pointDeVente.nom, quantite: s2.quantite, quantiteBloquee: s2.quantiteBloquee, quantiteConsignee: s2.quantiteConsignee })}
...
function BlocageStockModal({ stock, onClose, onDone }: {
