...
function KpiCard({ label, value, sub, icon: Icon, color }: {
  ...
  const h = STAT_CARD_HUES[hue] ?? STAT_CARD_HUES.slate;
  return (
    <div className={`group relative overflow-hidden ${h.solid} rounded-2xl p-4 sm:p-5 min-w-0 shadow-sm border transition-all duration-300 ease-out hover:-translate-y-1 hover:shadow-xl`}>
      <div className="flex items-start justify-between mb-3">
        <div className="bg-white/20 text-white [&_svg]:text-white p-2.5 rounded-xl transition-transform duration-300 ease-out group-hover:scale-110"><Icon className="text-white w-5 h-5" /></div>
      </div>
      <p className="text-xs font-semibold mb-0.5 text-white/85">{label}</p>
      <p className={`text-xl sm:text-2xl leading-tight [overflow-wrap:anywhere] font-bold transition-transform duration-300 group-hover:scale-105 origin-left text-white`}>{value}</p>
