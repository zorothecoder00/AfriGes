...
interface Mission {
  ...
  rapport:        string | null;
  notes:          string | null;
  collaborateur:  ProfilRH;
  validePar:      { id: number; matricule: string; gestionnaire: { member: { nom: string; prenom: string } } | null } | null;
  createdAt:      string;
}
...
const WORKFLOW_ACTIONS: Record<string, { action: string; label: string; color: string }[]> = {
  CREE:     [
    { action: "VALIDER",  label: "Valider",  color: "bg-blue-600 text-white hover:bg-blue-700" },
    { action: "ANNULER",  label: "Annuler",  color: "bg-red-50 text-red-600 hover:bg-red-100 border border-red-200" },
  ],
  VALIDE:   [
    { action: "DEMARRER", label: "Démarrer", color: "bg-amber-600 text-white hover:bg-amber-700" },
    { action: "ANNULER",  label: "Annuler",  color: "bg-red-50 text-red-600 hover:bg-red-100 border border-red-200" },
  ],
...
export default function MissionsRHPage() {
  ...
  const { data: res, loading, refetch } = useApi<MissionsResponse>(
    `/api/responsableRH/missions?${params}`
  );
      ...
      <div className="p-6 space-y-6 max-w-6xl mx-auto">
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-0.5">Gestion des missions de l&apos;équipe</p>
          </div>
            ...
            <div className="divide-y divide-slate-100">
              {missions.map((m) => (
                <MissionRow
                  key={m.id}
                  mission={m}
                  onCloture={() => setClotureMission(m)}
                  onDetail={() => setSelected(m)}
                  onRefetch={refetch}
                />
              ))}
            </div>
...
function MissionRow({
  mission, onCloture, onDetail, onRefetch,
}: { mission: Mission; onCloture: () => void; onDetail: () => void; onRefetch: () => void }) {
  const { mutate, loading } = useMutation(`/api/responsableRH/missions/${mission.id}`, "PATCH");
  ...
  const handleAction = async (action: string) => {
    if (action === "CLOTURER") { onCloture(); return; }
    const result = await mutate({ action });
    if (result) {
      toast.success(
        action === "VALIDER" ? "Mission validée" :
        action === "DEMARRER" ? "Mission démarrée" : "Mission annulée"
      );
      onRefetch();
    }
  };
        ...
        <div className="flex items-start justify-between gap-2">
          <div className="min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              <button onClick={onDetail} className="text-sm font-semibold text-slate-900 hover:text-emerald-600 text-left">
                {mission.titre}
              </button>
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            <button onClick={onDetail} className="p-1.5 text-slate-400 hover:text-slate-600 opacity-0 group-hover:opacity-100">
...
function CreateMissionModal({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  const { mutate, loading } = useMutation("/api/responsableRH/missions", "POST");
  ...
  const [form, setForm] = useState({
    collaborateurId: "", titre: "", objectifs: "", livrables: "",
    destination: "", dateDepart: "", dateRetour: "", notes: "",
  });
        ...
        <div className="overflow-y-auto flex-1 p-6 space-y-4">
          ...
          <Field label="Livrables attendus">
            <textarea value={form.livrables} onChange={(e) => set("livrables", e.target.value)} rows={2}
              placeholder="Rapports, résultats attendus…"
              className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 resize-none" />
          </Field>
        </div>
        ...
        <div className="overflow-y-auto flex-1 p-6 space-y-4">
          ...
          {editMode ? (
            <div className="space-y-3">
              ...
              <Field label="Rapport / compte-rendu">
                <textarea value={form.rapport} onChange={(e) => set("rapport", e.target.value)} rows={3}
                  placeholder="URL ou texte du rapport…"
                  className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 resize-none" />
              </Field>
            </div>
          ) : (
            <div className="space-y-3">
              <DRow label="Date départ"      value={formatDate(mission.dateDepart)} />
              {mission.dateRetour    && <DRow label="Retour prévu"   value={formatDate(mission.dateRetour)} />}
              {mission.dateRetourReel && <DRow label="Retour réel"    value={formatDate(mission.dateRetourReel)} />}
              {mission.destination   && <DRow label="Destination"    value={mission.destination} icon={<MapPin className="w-3.5 h-3.5" />} />}
              {mission.objectifs     && <DRow label="Objectifs"      value={mission.objectifs}  multiline />}
              {mission.livrables     && <DRow label="Livrables"      value={mission.livrables}  multiline />}
              {mission.rapport       && <DRow label="Rapport"        value={mission.rapport}    multiline />}
              {mission.notes         && <DRow label="Notes"          value={mission.notes} />}
            </div>
          )}
        </div>
...
function ClotureModal({ mission, onClose, onCloture }: {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md">
        ...
        <div className="p-6 space-y-4">
          <p className="text-sm text-slate-600">Mission : <span className="font-medium">{mission.titre}</span></p>
          <Field label="Date de retour réelle *">
          ...
          <Field label="Rapport / compte-rendu">
            <textarea value={rapport} onChange={(e) => setRapport(e.target.value)} rows={4}
              placeholder="Synthèse, résultats, URL du rapport…"
              className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 resize-none" />
          </Field>
        </div>
        <div className="flex justify-end gap-3 px-6 py-4 border-t border-slate-200">
...
function DRow({ label, value, icon, multiline }: {
