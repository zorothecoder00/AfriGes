...
function ProcDetailModal({ proc, onClose, onRefresh }: {
  ...
  const { mutate: trigger, loading } = useMutation(`/api/responsableRH/disciplinaire/${proc.id}`, "PATCH");
...
function CreateProcModal({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  ...
  const { data: collabData } = useApi<{ data: CollabOption[] }>(
    collabSearch.length >= 2
      ? `/api/responsableRH/collaborateurs?search=${encodeURIComponent(collabSearch)}&limit=10`
      : null
  );
  ...
  const { mutate: trigger, loading } = useMutation("/api/responsableRH/disciplinaire");
...
export default function DisciplinairePage() {
  ...
  const { data, loading, refetch } = useApi<ProcsResponse>(`/api/responsableRH/disciplinaire?${params}`);
