...
interface CollaborateurRow {
  ...
  gestionnaire: {
    ...
    member: { id: number; nom: string; prenom: string; email: string; telephone: string | null; photo: string | null;
...
export default function CollaborateursRHPage() {
  ...
  const { data: res, loading, refetch } = useApi<Response>(
    `/api/responsableRH/collaborateurs?${query}`
  );
    ...
    <div className="min-h-screen bg-slate-50">
      <div className="p-6 space-y-6">
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-0.5">
              {res?.meta.total ?? 0} collaborateur{(res?.meta.total ?? 0) !== 1 ? "s" : ""} dans votre périmètre
            </p>
          </div>
          <div className="flex gap-2">
            <button onClick={refetch} className="flex items-center gap-2 px-3 py-2 text-sm text-slate-600 bg-white border border-slate-200 rounded-lg hover:bg-slate-50">
              <RefreshCw className="w-4 h-4" />
            </button>
            <Link href="/dashboard/user/responsablesRH/collaborateurs/nouveau"
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            { label: "Actifs",           value: stats?.totalActifs  ?? 0, icon: <UserCheck className="w-5 h-5 text-emerald-600" />, bg: "bg-emerald-50" },
            { label: "Période d'essai",  value: stats?.totalEnEssai ?? 0, icon: <Clock className="w-5 h-5 text-blue-600" />,     bg: "bg-blue-50"    },
            { label: "Total effectif",   value: res?.meta.total     ?? 0, icon: <Users className="w-5 h-5 text-slate-600" />,    bg: "bg-slate-100"  },
            { label: "Inactifs / partis",value: stats?.totalInactifs ?? 0, icon: <UserX className="w-5 h-5 text-red-500" />,     bg: "bg-red-50"     },
          ].map((s) => (
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-4 flex flex-wrap gap-3 items-center">
          ...
          <select value={typeContrat} onChange={(e) => { setTypeContrat(e.target.value); setPage(1); }}
            className="px-3 py-2 border border-slate-200 rounded-lg text-sm bg-white focus:outline-none">
            <option value="">Tous les contrats</option>
            {["CDI","CDD","STAGE","CONSULTANT","PRESTATAIRE","FREELANCE"].map((c) => <option key={c} value={c}>{c}</option>)}
          </select>
          <div className="flex items-center gap-1 text-sm text-slate-500">
            <Filter className="w-4 h-4" /> {res?.meta.total ?? 0} résultat(s)
          </div>
        </div>
          ...
          {loading ? (
            ...
            <table className="w-full text-sm">
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr>
                  {["Collaborateur","Matricule","Poste / Service","Contrat","PDV","Embauche","Statut",""].map((h) => (
                    ...
                    <tr key={c.id} className="hover:bg-slate-50 transition-colors">
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <div className="font-semibold text-slate-800">{c.gestionnaire.member.prenom} {c.gestionnaire.member.nom}</div>
                            <div className="flex items-center gap-2 mt-0.5">
                              {c.gestionnaire.member.email && (
                                <span className="flex items-center gap-1 text-[11px] text-slate-400">
                                  <Mail className="w-3 h-3" />{c.gestionnaire.member.email}
                                </span>
                              )}
                              {c.gestionnaire.member.telephone && (
                                <span className="flex items-center gap-1 text-[11px] text-slate-400">
                                  <Phone className="w-3 h-3" />{c.gestionnaire.member.telephone}
                                </span>
                              )}
                            </div>
                          </div>
                      ...
                      <td className="px-4 py-3">
                        {c.fonction && <div className="flex items-center gap-1 text-sm text-slate-700"><Briefcase className="w-3.5 h-3.5 text-slate-400" />{c.fonction}</div>}
                        {c.service  && <div className="text-xs text-slate-400 mt-0.5">{c.service}</div>}
                        {!c.fonction && !c.service && <span className="text-xs text-slate-300">—</span>}
                      </td>
                      <td className="px-4 py-3">
                        {c.typeContrat ? (
                          <span className={`inline-block px-2 py-0.5 rounded-full text-xs font-semibold ${CONTRAT_BADGE[c.typeContrat] ?? "bg-gray-100 text-gray-600"}`}>{c.typeContrat}</span>
                        ) : <span className="text-xs text-slate-300">—</span>}
                      </td>
                      <td className="px-4 py-3">
                        {pdv ? (
                          <div className="flex items-center gap-1 text-xs text-slate-500"><Building2 className="w-3 h-3 text-slate-400" />{pdv.nom}</div>
                        ) : <span className="text-xs text-slate-300">—</span>}
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <Link href={`/dashboard/user/responsablesRH/collaborateurs/${c.id}`}
          ...
          <div className="flex items-center justify-between text-sm text-slate-600">
            ...
            <div className="flex gap-2">
              <button disabled={page === 1} onClick={() => setPage((p) => p - 1)}
                className="px-3 py-1.5 border border-slate-200 rounded-lg disabled:opacity-40 hover:bg-slate-50">Précédent</button>
              <button disabled={page === res.meta.totalPages} onClick={() => setPage((p) => p + 1)}
                className="px-3 py-1.5 border border-slate-200 rounded-lg disabled:opacity-40 hover:bg-slate-50">Suivant</button>
            </div>
          </div>
        )}
