...
function fmt(n: number) { return n.toLocaleString("fr-FR", { minimumFractionDigits: 0, maximumFractionDigits: 0 }) + " FCFA"; }
...
export default function RecouvrementPage() {
  ...
  const agentsDisponibles = useMemo(() => {
    ...
    for (const f of data.financements) { if (f.client.agentTerrain) seen.add(f.client.agentTerrain); }
    return [...seen].sort();
  }, [data]);
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex items-center justify-end gap-1.5">
                        <div className="w-16 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                          <div className={`h-full rounded-full ${pf.tauxRecouvrement >= 80 ? "bg-emerald-500" : pf.tauxRecouvrement >= 50 ? "bg-amber-500" : "bg-red-500"}`}
        ...
        <div className="space-y-4">
          <div className="flex flex-wrap gap-3">
            <div className="relative flex-1 min-w-48">
              ...
              <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Chercher client, référence…"
            ...
            {tab === "clients" && (
              <select value={filtreAlerte} onChange={(e) => setFiltreAlerte(e.target.value as NiveauAlerte | "TOUS")}
              ...
              <table className="w-full text-sm">
                <thead className="bg-slate-50 text-xs text-slate-500 uppercase">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-right cursor-pointer hover:text-slate-700 select-none" onClick={() => toggleSort("encours")}>
                      <span className="flex items-center justify-end gap-1">
                        Encours {sortField === "encours" ? (sortDir === "desc" ? <ChevronDown className="w-3 h-3" /> : <ChevronUp className="w-3 h-3" />) : null}
                      </span>
                    </th>
                    <th className="px-4 py-3 text-center cursor-pointer hover:text-slate-700 select-none" onClick={() => toggleSort("joursRetard")}>
                      <span className="flex items-center justify-center gap-1">
                        Retard {sortField === "joursRetard" ? (sortDir === "desc" ? <ChevronDown className="w-3 h-3" /> : <ChevronUp className="w-3 h-3" />) : null}
                      </span>
                    </th>
                  ...
                  {financementsFiltres.map((f) => {
                    ...
                    return (
                      <tr key={f.id} className={`hover:bg-slate-50 ${f.niveauAlerte === "J30+" ? "bg-red-50/30" : ""}`}>
                        ...
                        <td className="px-4 py-3 text-xs text-slate-500">{f.client.agentTerrain ?? <span className="text-slate-300">—</span>}</td>
