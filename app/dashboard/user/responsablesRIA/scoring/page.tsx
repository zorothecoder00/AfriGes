...
interface AffectationScore {
  ...
  client: { id: number; nom: string; telephone: string | null; niveauRisque: string | null; scoreSolvabilite: number | null };
            ...
            return (
              <div key={cls} className="flex items-center gap-3">
                <button onClick={() => setFiltreClasse(filtreClasse === cls ? "TOUS" : cls)}
                  className={`flex items-center gap-1.5 w-32 text-xs font-medium px-2 py-1 rounded-md border transition-colors ${filtreClasse === cls ? cfg.bg + " " + cfg.color : "border-transparent text-slate-500 hover:bg-slate-50"}`}>
                  <Icon className="w-3 h-3" />{cls}<span className="ml-auto text-slate-400">{count}</span>
                </button>
                <div className="flex-1 h-2 bg-slate-100 rounded-full overflow-hidden">
                  <div className={`h-full rounded-full transition-all ${cfg.barColor}`} style={{ width: `${(count / maxRepartition) * 100}%` }} />
                </div>
                ...
                return (
                  <tr key={aff.id} className={`hover:bg-slate-50 ${diverge ? "bg-amber-50/30" : ""}`}>
                    ...
                    <td className="px-4 py-3 text-center">
                      <span className={`px-2 py-0.5 text-xs font-bold rounded-md ${BADGE[aff.classeRisque]}`}>{aff.classeRisque}</span>
                    </td>
                    <td className="px-4 py-3 text-center">
                      <span className={`px-2 py-0.5 text-xs font-bold rounded-md ${BADGE[aff._classeCalculee]}`}>{aff._classeCalculee}</span>
                      {diverge && <p className="text-xs text-amber-600 mt-0.5">À mettre à jour</p>}
                    </td>
                    ...
                    <td className="px-4 py-3 text-center">
                      {aff.client.scoreSolvabilite !== null
                        ? <span className={`text-xs font-medium ${aff.client.scoreSolvabilite >= 75 ? "text-emerald-600" : aff.client.scoreSolvabilite >= 50 ? "text-amber-600" : "text-red-600"}`}>
        ...
        {affectationsFiltrees.length > 0 && (
          <div className="px-4 py-2.5 border-t border-slate-100 text-xs text-slate-400">
            ...
            {affectationsFiltrees.filter((a) => a.classeRisque !== a._classeCalculee).length > 0 && (
              <span className="ml-2 text-amber-600">· {affectationsFiltrees.filter((a) => a.classeRisque !== a._classeCalculee).length} à recalculer</span>
            )}
          </div>
        )}
      </div>
