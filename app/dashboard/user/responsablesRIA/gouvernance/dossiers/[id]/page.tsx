...
export default function DossierDetailPage() {
  ...
  const { mutate: mutatePatch, loading: patching } = useMutation(`/api/membreCommission/dossiers/${id}`, "PATCH");
  const { mutate: mutateEchange, loading: envoiEnCours } = useMutation(`/api/membreCommission/dossiers/${id}/echanges`, "POST");
        ...
        <div className="rounded-xl border border-emerald-300 bg-emerald-50 p-4 flex items-start gap-3">
          ...
          {peutDecaisser ? (
            <button onClick={() => runAction({ action: "EXECUTER" })}
              ...
              <Wallet className="w-4 h-4" /> {patching ? "Décaissement..." : "Décaisser maintenant"}
            </button>
          ) : (
