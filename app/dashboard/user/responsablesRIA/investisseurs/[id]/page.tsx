...
function AddPortefeuilleModal({ profilRIAId, onClose, onSuccess }: {
  ...
  const mut = useMutation<{ data: unknown }, { profilRIAId: number; nom: string; notes: string }>(
    "/api/admin/ria/portefeuilles", "POST"
  );
    ...
    <div className="border border-slate-200 rounded-2xl overflow-hidden">
      <div className="bg-slate-50 px-5 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <Link href={`/dashboard/user/responsablesRIA/portefeuilles/${pf.id}`}
...
export default function FicheInvestisseurPage() {
  ...
  const { data: res, loading, refetch } = useApi<{ data: Investisseur }>(
    `/api/admin/ria/investisseurs/${params.id}`
  );
  ...
  if (!inv || !m) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-slate-400 gap-3">
        ...
        <Link href="/dashboard/user/responsablesRIA/investisseurs" className="text-emerald-600 hover:underline text-sm">
          Retour à la liste
        </Link>
      </div>
    );
  ...
  return (
    <div className="p-6 space-y-6 max-w-5xl mx-auto">
      ...
      <div className="flex items-center gap-4">
        <Link href="/dashboard/user/responsablesRIA/investisseurs"
        ...
        <div className="ml-auto flex items-center gap-2">
          <Link href={`/dashboard/user/responsablesRIA/investisseurs/${m.id}/documents`}
