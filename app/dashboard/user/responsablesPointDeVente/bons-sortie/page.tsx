...
/**
 * Bons de sortie de l'agence du RPV (CDC digitalisation §3.4) — dont ceux remplis par les
 * agents terrain. Le RPV supervise et vise les bons au-delà du seuil ; l'exécution (avec
 * ajustement des quantités) reste au magasinier.
 */
...
export default function BonsSortieRPVPage() {
  ...
  const { data, loading, refetch } = useApi<{ data: BonSortie[]; pdv: { nom: string; code: string }; seuilVisaBonSortie: number }>(`/api/rpv/bons-sortie?${params}`);
      ...
      <div className="p-4 md:p-6 max-w-5xl mx-auto space-y-4">
        ...
        <div className="flex items-start justify-between gap-3 flex-wrap">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-0.5">
              {data?.pdv ? `${data.pdv.nom} — ` : ""}sorties de marchandises de votre agence, dont celles demandées par les agents terrain. L&apos;exécution revient au magasinier ; vous visez les bons au-delà du seuil.
            </p>
          </div>
        ...
        <div className="flex gap-2 flex-wrap">
          <select value={origine} onChange={(e) => setOrigine(e.target.value)} className={selectCls}>
            ...
            <option value="">Tous les bons de l&apos;agence</option>
          </select>
                  ...
                  <div className="flex items-start justify-between gap-3 flex-wrap">
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-slate-400 mt-0.5">
                        {montant.toLocaleString("fr-FR")} FCFA · rempli par {b.creePar.prenom} {b.creePar.nom} · {new Date(b.createdAt).toLocaleDateString("fr-FR")}
