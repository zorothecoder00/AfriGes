import { NextResponse } from "next/server";
import { prisma } from "@/lib/prisma";
import { getLogistiqueSession } from "@/lib/authLogistique";
import { genererGrandLivreFournisseur } from "@/lib/comptabilite/clientAuxiliaire";

type Ctx = { params: Promise<{ id: string }> };

/**
 * GET /api/logistique/fournisseurs/[id]/releve?dateDebut=&dateFin=
 * Relevé fournisseur (CDC digitalisation §5.3) — même moteur que le relevé
 * comptable (lib/comptabilite/clientAuxiliaire.ts), ouvert ici au rôle
 * Logistique/Approvisionnement (gère la relation fournisseur au quotidien,
 * n'avait jusqu'ici pas accès au relevé — réservé au Comptable).
 */
export async function GET(req: Request, { params }: Ctx) {
  try {
    const session = await getLogistiqueSession();
    if (!session) return NextResponse.json({ error: "Accès refusé" }, { status: 403 });

    const { id } = await params;
    const fournisseurId = Number(id);
    if (isNaN(fournisseurId)) return NextResponse.json({ error: "ID invalide" }, { status: 400 });

    const { searchParams } = new URL(req.url);
    const dateDebut = searchParams.get("dateDebut") ? new Date(searchParams.get("dateDebut")!) : null;
    const dateFin = searchParams.get("dateFin") ? new Date(`${searchParams.get("dateFin")}T23:59:59`) : new Date();

    const fournisseur = await prisma.fournisseur.findUnique({
      where: { id: fournisseurId },
      select: { id: true, nom: true, code: true, contact: true, telephone: true, adresse: true },
    });
    if (!fournisseur) return NextResponse.json({ error: "Fournisseur introuvable" }, { status: 404 });

    const grandLivre = await genererGrandLivreFournisseur(prisma, fournisseurId, { dateDebut, dateFin });

    return NextResponse.json({
      data: { fournisseur, periode: { debut: dateDebut, fin: dateFin }, ...grandLivre },
    });
  } catch (e) {
    console.error("GET /logistique/fournisseurs/[id]/releve:", e);
    return NextResponse.json({ error: "Erreur serveur" }, { status: 500 });
  }
}
