...
/**
 * GET /api/responsableRH/collaborateurs/[id]/documents
 * Liste les documents d'un collaborateur (dans le périmètre PDV du RH)
 */
export async function GET(_req: NextRequest, { params }: Ctx) {
  try {
    ...
    if (!(await profilRHDansPerimetre(session, Number(id)))) {
    ...
    console.error("GET /api/responsableRH/collaborateurs/[id]/documents", error);
    return NextResponse.json({ error: "Erreur serveur" }, { status: 500 });
  }
}

/**
 * POST /api/responsableRH/collaborateurs/[id]/documents
  ...
  try {
    ...
    await prisma.auditLog.create({
      data: {
        ...
        entiteId: doc.id,
        details:  `Document "${nom}" (${type} v${version}) ajouté au dossier #${id} par RH`,
      },
    });

    return NextResponse.json({ data: doc }, { status: 201 });
  } catch (error) {
    console.error("POST /api/responsableRH/collaborateurs/[id]/documents", error);
    return NextResponse.json({ error: "Erreur serveur" }, { status: 500 });
  }
}
