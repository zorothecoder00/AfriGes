...
/**
 * GET /api/responsableRH/evaluations/[id]/actions
...
export async function GET(_req: NextRequest, { params }: Ctx) {
  try {
    ...
    const evaluation = await prisma.evaluationRH.findUnique({ where: { id: Number(id) }, select: { profilRHId: true } });
    ...
    console.error("GET /api/responsableRH/evaluations/[id]/actions", error);
    return NextResponse.json({ error: "Erreur serveur" }, { status: 500 });
  }
}

/**
 * POST /api/responsableRH/evaluations/[id]/actions
...
export async function POST(req: NextRequest, { params }: Ctx) {
  try {
    ...
    console.error("POST /api/responsableRH/evaluations/[id]/actions", error);
    return NextResponse.json({ error: "Erreur serveur" }, { status: 500 });
  }
}
