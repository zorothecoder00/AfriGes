...
/**
 * PATCH /api/responsableRH/evaluations/[id]/actions/[actionId]
...
export async function PATCH(req: NextRequest, { params }: Ctx) {
  try {
    ...
    if (!(await profilRHDansPerimetre(session, existing.profilRHId))) {
      return NextResponse.json({ error: "Action hors de votre périmètre" }, { status: 403 });
    }
    ...
    console.error("PATCH /api/responsableRH/evaluations/[id]/actions/[actionId]", error);
    return NextResponse.json({ error: "Erreur serveur" }, { status: 500 });
  }
}

/**
 * DELETE /api/responsableRH/evaluations/[id]/actions/[actionId]
 */
export async function DELETE(_req: NextRequest, { params }: Ctx) {
  try {
    ...
    console.error("DELETE /api/responsableRH/evaluations/[id]/actions/[actionId]", error);
    return NextResponse.json({ error: "Erreur serveur" }, { status: 500 });
  }
}
