...
/**
 * GET /api/responsableRH/evaluations/[id]/objectifs
...
export async function GET(_req: NextRequest, { params }: Ctx) {
  try {
    ...
    if (!(await profilRHDansPerimetre(session, eval_.profilRHId))) {
    ...
    console.error("GET /api/responsableRH/evaluations/[id]/objectifs", error);
    return NextResponse.json({ error: "Erreur serveur" }, { status: 500 });
  }
}

/**
 * POST /api/responsableRH/evaluations/[id]/objectifs
...
export async function POST(req: NextRequest, { params }: Ctx) {
  try {
    ...
    console.error("POST /api/responsableRH/evaluations/[id]/objectifs", error);
    return NextResponse.json({ error: "Erreur serveur" }, { status: 500 });
  }
}

/**
 * PATCH /api/responsableRH/evaluations/[id]/objectifs
...
export async function PATCH(req: NextRequest, { params }: Ctx) {
  try {
    ...
    const eval_ = await prisma.evaluationRH.findUnique({ where: { id: Number(id) }, select: { profilRHId: true } });
    if (!eval_ || !(await profilRHDansPerimetre(session, eval_.profilRHId))) {
      return NextResponse.json({ error: "Évaluation hors de votre périmètre" }, { status: 403 });
    }
    ...
    console.error("PATCH /api/responsableRH/evaluations/[id]/objectifs", error);
    return NextResponse.json({ error: "Erreur serveur" }, { status: 500 });
  }
}

/**
 * DELETE /api/responsableRH/evaluations/[id]/objectifs
...
export async function DELETE(req: NextRequest, { params }: Ctx) {
  try {
    ...
    console.error("DELETE /api/responsableRH/evaluations/[id]/objectifs", error);
    return NextResponse.json({ error: "Erreur serveur" }, { status: 500 });
  }
}
