...
/**
 * GET /api/responsableRH/organigramme
...
export async function GET() {
  try {
    ...
    console.error("GET /api/responsableRH/organigramme", error);
    return NextResponse.json({ error: "Erreur serveur" }, { status: 500 });
  }
}

/**
 * PATCH /api/responsableRH/organigramme
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    if (!(await profilRHDansPerimetre(session, profil.id))) {
    ...
    console.error("PATCH /api/responsableRH/organigramme", error);
    return NextResponse.json({ error: "Erreur serveur" }, { status: 500 });
  }
}
