...
export async function GET(_req: NextRequest, { params }: Ctx) {
  try {
    ...
    if (!(await profilRHDansPerimetre(session, proc.profilRHId))) {
      return NextResponse.json({ error: "Procédure hors de votre périmètre" }, { status: 403 });
    }
    return NextResponse.json({ data: proc });
  } catch (error) {
    console.error("GET /api/responsableRH/disciplinaire/[id]", error);
    return NextResponse.json({ error: "Erreur serveur" }, { status: 500 });
  }
}

/**
 * PATCH /api/responsableRH/disciplinaire/[id]
...
export async function PATCH(req: NextRequest, { params }: Ctx) {
  try {
    ...
    console.error("PATCH /api/responsableRH/disciplinaire/[id]", error);
    return NextResponse.json({ error: "Erreur serveur" }, { status: 500 });
  }
}
