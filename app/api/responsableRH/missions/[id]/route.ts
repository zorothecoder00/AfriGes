...
/**
 * GET /api/responsableRH/missions/[id]
 */
export async function GET(_req: NextRequest, { params }: Ctx) {
  try {
    ...
    const mission = await prisma.mission.findUnique({ where: { id: Number(id) }, include: INCLUDE });
    if (!mission) return NextResponse.json({ error: "Mission introuvable" }, { status: 404 });

    return NextResponse.json({ data: mission });
  } catch (error) {
    console.error("GET /api/responsableRH/missions/[id]", error);
    return NextResponse.json({ error: "Erreur serveur" }, { status: 500 });
  }
}

/**
 * PATCH /api/responsableRH/missions/[id]
 * Workflow : { action: "VALIDER" | "DEMARRER" | "CLOTURER" | "ANNULER", rapport?, dateRetourReel? }
 * Édition  : { titre?, objectifs?, livrables?, destination?, dateDepart?, dateRetour?, notes?, rapport? }
 */
  ...
  try {
    ...
    for (const key of allowed) {
      if (key in editFields) {
        updateData[key] = (key === "dateDepart" || key === "dateRetour")
          ? (editFields[key] ? new Date(editFields[key]) : null)
          : (editFields[key] ?? null);
      }
    }
    ...
    const updated = await prisma.mission.update({ where: { id: Number(id) }, data: updateData, include: INCLUDE });
    return NextResponse.json({ data: updated });
  } catch (error) {
    console.error("PATCH /api/responsableRH/missions/[id]", error);
    return NextResponse.json({ error: "Erreur serveur" }, { status: 500 });
  }
}
