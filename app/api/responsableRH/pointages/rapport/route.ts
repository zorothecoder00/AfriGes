...
/**
 * GET /api/responsableRH/pointages/rapport — Rapport mensuel de présence, scopé
 * au périmètre PDV. Même agrégat que /api/admin/rh/pointages/rapport (cf. ce
 * fichier pour le détail des champs retournés).
 *   Query: mois (1-12), annee, profilRHId?, departement?
 */
export async function GET(req: NextRequest) {
  try {
    ...
    if (profilRHIds)  profilWhere.id          = { in: profilRHIds };
    ...
    console.error("GET /api/responsableRH/pointages/rapport", error);
