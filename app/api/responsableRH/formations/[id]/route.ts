...
/**
 * GET /api/responsableRH/formations/[id]
 */
export async function GET(_req: NextRequest, { params }: Ctx) {
  try {
    ...
    console.error("GET /api/responsableRH/formations/[id]", error);
    return NextResponse.json({ error: "Erreur serveur" }, { status: 500 });
  }
}

/**
 * PATCH /api/responsableRH/formations/[id]
  ...
  try {
    ...
    if (addParticipants || removeParticipants) {
      if (addParticipants?.length) {
        ...
        if (perimetre && addParticipants.some((pid: number) => !perimetre.includes(pid))) {
          return NextResponse.json({ error: "Un ou plusieurs collaborateurs sont hors de votre périmètre" }, { status: 403 });
        }
      }
    ...
    console.error("PATCH /api/responsableRH/formations/[id]", error);
    return NextResponse.json({ error: "Erreur serveur" }, { status: 500 });
  }
}
