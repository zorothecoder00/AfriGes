...
export async function GET(_req: NextRequest, { params }: Ctx) {
  try {
    ...
    console.error("GET /api/responsableRH/sst/incidents/[id]", error);
    return NextResponse.json({ error: "Erreur serveur" }, { status: 500 });
  }
}

/**
 * PATCH /api/responsableRH/sst/incidents/[id]
...
export async function PATCH(req: NextRequest, { params }: Ctx) {
  try {
    ...
    console.error("PATCH /api/responsableRH/sst/incidents/[id]", error);
    return NextResponse.json({ error: "Erreur serveur" }, { status: 500 });
  }
}
