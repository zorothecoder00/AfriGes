...
/**
 * GET /api/responsableRH/sst/accidents
...
export async function GET(req: NextRequest) {
  try {
    ...
    console.error("GET /api/responsableRH/sst/accidents", error);
    return NextResponse.json({ error: "Erreur serveur" }, { status: 500 });
  }
}

/**
 * POST /api/responsableRH/sst/accidents
...
export async function POST(req: NextRequest) {
  try {
    ...
    const perimetre = await profilRHIdsPerimetre(session);
    if (perimetre && !perimetre.includes(Number(profilRHId))) {
    ...
    console.error("POST /api/responsableRH/sst/accidents", error);
    return NextResponse.json({ error: "Erreur serveur" }, { status: 500 });
  }
}
