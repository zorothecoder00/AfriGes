...
export async function GET(_req: NextRequest, { params }: Ctx) {
  try {
    ...
    if (!(await profilRHDansPerimetre(session, accident.profilRHId))) {
      return NextResponse.json({ error: "Accident hors de votre périmètre" }, { status: 403 });
    }
    return NextResponse.json({ data: accident });
  } catch (error) {
    console.error("GET /api/responsableRH/sst/accidents/[id]", error);
    return NextResponse.json({ error: "Erreur serveur" }, { status: 500 });
  }
}

/**
 * PATCH /api/responsableRH/sst/accidents/[id]
...
export async function PATCH(req: NextRequest, { params }: Ctx) {
  try {
    ...
    console.error("PATCH /api/responsableRH/sst/accidents/[id]", error);
    return NextResponse.json({ error: "Erreur serveur" }, { status: 500 });
  }
}
