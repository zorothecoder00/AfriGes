...
/**
 * PATCH /api/collaborateur/formations/[id]
 * Seule action autorisée pour le collaborateur : ANNULER sa propre demande,
 * tant qu'elle n'a pas reçu sa décision finale.
 *
 * Body: { action: "ANNULER" }
 */
export async function PATCH(req: NextRequest, { params }: Ctx) {
  try {
    ...
    const updated = await prisma.demandeFormation.update({
      where: { id: Number(id) },
    ...
    console.error("PATCH /api/collaborateur/formations/[id]", error);
