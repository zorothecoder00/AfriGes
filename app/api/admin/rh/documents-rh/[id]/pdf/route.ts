...
/**
 * GET /api/admin/rh/documents-rh/[id]/pdf
 * Document RH généré (attestation, certificat, décision, lettre de mission) en
 * PDF (streaming). Réutilise le HTML déjà stocké (`contenu`).
 */
export async function GET(_req: Request, { params }: Ctx) {
  try {
    ...
    const doc = await prisma.documentRHGenere.findUnique({
      where:  { id: Number(id) },
    ...
    console.error("GET /api/admin/rh/documents-rh/[id]/pdf", error);
