...
/**
 * GET  /api/admin/rh/paie/prets
 *   Query: profilRHId?, statut?, page?, limit?
 *
 * POST /api/admin/rh/paie/prets
 *   Body: { profilRHId, montant, tauxInteret?, dureesMois, notes? }
 *   Calcule montantMensuel = (montant * (1 + taux/100)) / dureesMois
 */
...
export async function GET(req: NextRequest) {
  try {
    ...
    console.error("GET /api/admin/rh/paie/prets", error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    console.error("POST /api/admin/rh/paie/prets", error);
    return NextResponse.json({ error: "Erreur serveur" }, { status: 500 });
  }
}
