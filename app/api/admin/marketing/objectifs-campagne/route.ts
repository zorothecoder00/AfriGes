...
/**
 * GET/POST /api/admin/marketing/objectifs-campagne
 * Référentiel paramétrable sans code (CDC §81) : objectifs de campagne
 * (remplace l'ex-enum ObjectifCampagneType, jamais branché nulle part).
 */
export async function GET() {
  ...
  const objectifs = await prisma.objectifCampagne.findMany({ orderBy: [{ ordre: "asc" }, { libelle: "asc" }] });
...
export async function POST(req: Request) {
  ...
  try {
    const objectif = await prisma.objectifCampagne.create({
      data: { code: String(code).toUpperCase().replace(/\s+/g, "_"), libelle, ordre: ordre ? Number(ordre) : 0 },
    });
    return NextResponse.json({ data: objectif }, { status: 201 });
  } catch {
