 ...
 * Liste des anomalies en attente de validation admin (statut TRANSMISE)
 * + toutes les anomalies avec filtres.
  ...
  try {
    ...
    const [anomalies, total, pendingCount] = await Promise.all([
      prisma.anomalieStock.findMany({
        ...
        include: {
          produit:      { select: { id: true, nom: true, reference: true, prixAchat: true, prixUnitaire: true } },
      ...
      prisma.anomalieStock.count({ where: { statut: "TRANSMISE" } }),
    ]);
    ...
    console.error("GET /admin/anomalies:", error);
