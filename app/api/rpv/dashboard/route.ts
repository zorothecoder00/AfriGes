 ...
 *  - Versements du jour (ex-ventes creditAlimentaire → VersementPack)
...
export async function GET() {
  try {
    ...
    const [
      ...
      // Versements du jour (remplace venteCreditAlimentaire)
    ...
    const ventesParHeure = Array.from({ length: 24 }, (_, h) => {
      ...
      return {
        heure:   h,
        count:   hvs.length,
        montant: hvs.reduce((s, v) => s + Number(v.montant), 0),
      };
    });
