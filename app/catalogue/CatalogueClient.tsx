 ...
 *
 * Reçoit un premier rendu depuis le serveur (`app/catalogue/page.tsx`, ISR) pour
 * le SEO et l'affichage immédiat ; ne refetch en client qu'à partir d'une
 * interaction utilisateur (recherche, filtre, pagination).
 */
...
type Refs = { familles: Ref[]; marques: Ref[] };
...
export default function CatalogueClient({
  ...
  const [res, setRes]         = useState<Reponse | null>(initialData);
  const [loading, setLoading] = useState(false);
  const [refs]                = useState<Refs>(initialRefs);

  // Le tout premier rendu vient déjà du serveur (page 1, sans filtre) : on ne
  // refetch pas immédiatement en client, seulement dès qu'un filtre change.
  ...
  useEffect(() => {
    const estRequeteInitiale = page === 1 && !debounced && !familleId && !marqueId;
    if (isFirstFetch.current) {
      isFirstFetch.current = false;
      if (estRequeteInitiale && initialData) return; // déjà rendu par le serveur
    }
    charger();
