...
export const AIDE_COMPTABLE: Record<string, AideComptableContenu> = {
  ...
  configInitiale: {
    ...
    referentiel: "Devises, multi-sociétés, assistant de configuration initiale.",
  ...
  saisie: {
    ...
    referentiel: "Partie double, statuts, contrepassation.",
  ...
  tresorerie: {
    ...
    referentiel: "Flux de trésorerie.",
  ...
  balance: {
    ...
    regle: "Calculée uniquement à partir des lignes d'écritures VALIDE/CLOTURE, jamais des modules métier directement.",
  ...
  tva: {
    ...
    referentiel: "Taxes génériques paramétrables ; fiscalité togolaise.",
  ...
  rapprochement: {
    ...
    referentiel: "Rapprochement bancaire, import CSV/Excel/OFX.",
  ...
  etats: {
    ...
    referentiel: "Bilan, compte de résultat, flux de trésorerie, notes annexes ; SYSCOHADA révisé.",
  ...
  pieces: {
    ...
    referentiel: "Piste d'audit, conservation des pièces justificatives.",
    exemple: "Joindre le PDF de la facture fournisseur à l'écriture d'achat correspondante.",
  },
  regles: {
    titre: "Règles comptables",
    fonction: "Moteur central : associe chaque type d'événement métier (vente validée, paie versée, remboursement confirmé, réception d'achat, paiement fournisseur...) à un compte débit et un compte crédit, avec conditions optionnelles (produit, famille, mode de paiement) et priorité.",
    regle: "S'il n'existe pas de règle personnalisée active pour un événement, le moteur retombe automatiquement sur une règle par défaut (comportement historique conservé, zéro régression). Jamais de comptes câblés en dur dans le code applicatif. Un achat réceptionné « à crédit » crédite le fournisseur (401) ; marqué « comptant », il crédite directement la trésorerie (571/521) sans jamais transiter par une dette fournisseur fictive.",
    referentiel: "Moteur de règles comptables paramétrable, intégration des modules métier.",
    exemple: "Règle « VENTE_CREDIT_VALIDEE » : Dr 411 Client / Cr 701 Ventes, priorité 10. « RECEPTION_ACHAT_VALIDEE » comptant espèces : Dr 311 Marchandises / Cr 571 Caisse. « SORTIE_STOCK_VENTE » (COGS) : Dr 6031 Variation de stocks / Cr 311 Marchandises, constatée à la sortie physique du stock (vente comptant ou livraison crédit confirmée).",
  },
  auxiliaire: {
    ...
    fonction: "Suit le solde de chaque client/fournisseur individuellement, permet le lettrage (rapprocher une facture avec son règlement), et affiche la balance âgée (solde dû ventilé 0-30/31-60/61-90/90j+).",
    regle: "La balance âgée ne regarde que les lignes NON lettrées : une facture déjà rapprochée de son paiement n'y apparaît plus, exactement comme un impayé réel.",
    referentiel: "Comptes auxiliaires, lettrage, balance âgée.",
    exemple: "Lettrer la facture client #123 (Dr 411) avec le règlement reçu le mois suivant (Cr 411) : les deux lignes portent alors la même lettre et disparaissent de la balance âgée.",
  },
  immobilisations: {
    ...
    referentiel: "Immobilisations et amortissements ; SYSCOHADA (linéaire/dégressif).",
  ...
  analytique: {
    ...
    referentiel: "Comptabilité analytique 5 axes, budget.",
  ...
  etatsReels: {
    ...
    referentiel: "États financiers et rapports de gestion.",
  },
  controles: {
    ...
    referentiel: "Contrôles de cohérence, comptes d'attente, détection de doublons.",
  ...
  importEcritures: {
    ...
    referentiel: "Import/export comptable.",
  ...
  rapportsGestion: {
    ...
    referentiel: "Rapports de gestion détaillés.",
