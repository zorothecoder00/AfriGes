...
async function collecterLignesVente(tx: TxClient, dateDebut: Date, dateFin: Date): Promise<LigneVenteAgregat[]> {
  const [lignesVD, lignesCredit] = await Promise.all([
    tx.ligneVenteDirecte.findMany({
      ...
      select: { produitId: true, quantite: true, montant: true, vente: { select: { pointDeVenteId: true } } },
    }),
    tx.ligneCreditClient.findMany({
      ...
      select: { produitId: true, quantite: true, montantLigne: true, pointDeVenteId: true },
    }),
  ]);
  ...
  for (const l of lignesVD) {
    ...
    lignes.push({ pdvId: l.vente.pointDeVenteId, produitId: l.produitId, quantite: l.quantite, ca: Number(l.montant) });
  }
  for (const l of lignesCredit) {
    if (!l.produitId || !l.pointDeVenteId) continue;
    lignes.push({ pdvId: l.pointDeVenteId, produitId: l.produitId, quantite: l.quantite, ca: Number(l.montantLigne) });
  }
  return lignes;
}
...
export async function genererRapportsGestion(tx: TxClient, dateDebut: Date, dateFin: Date): Promise<RapportsGestion> {
  ...
  return { caParPdv, margeParProduit, margeParFamille, rentabiliteParAgence, rotationStock, dso, dpoFournisseursProxy };
}
